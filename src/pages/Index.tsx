...
import { Play, Pause, BookOpen, Brain, Lightbulb, Users } from "lucide-react";
...
const Index = () => {
  ...
  // SEO single H1 + structured data
  useEffect(() => {
    document.title = `${episodeData.Title} ｜ 兒童科普伴讀`;
  }, []);
    ...
    <main className="text-foreground">
      ...
      <SectionShell id="s1" show ref={refs[0]}>
        <div className="text-center">
          <motion.h1
            ...
            className="text-3xl sm:text-4xl font-black text-secondary leading-snug mb-6 text-stroke-dark"
          >
            <span className="block text-accent text-base sm:text-lg mb-2">🎧 今日科普探險</span>
            {episodeData.Title}
          </motion.h1>
          ...
          <motion.div
            ...
            className="relative rounded-3xl overflow-hidden shadow-[var(--shadow-card)] border-4 border-secondary/30 mb-6"
          >
            <img
          ...
          <div className="text-left mt-10">
            <h3 className="flex items-center gap-2 text-xl font-extrabold text-accent mb-4">
              <BookOpen className="w-6 h-6" />
              聽故事時有不懂的詞嗎？
            </h3>
            <p className="text-muted-foreground text-sm mb-4">點一下卡片，看看這些詞是什麼意思 👇</p>
      ...
      >
        <div className="glass-card rounded-3xl p-5 sm:p-7 mb-5">
          ...
          <p className="leading-relaxed text-base sm:text-lg">{audio.description}</p>
        </div>

        <Collapsible label="看看博士怎麼說">
          <div className="markdown-body text-foreground/90 text-sm sm:text-base">
        ...
        <div className="space-y-4">
          {episodeData.KeyTakeaway.map((k, i) => (
            <motion.div
              ...
              className="glass-card rounded-2xl p-5 flex gap-4 items-start hover:border-secondary transition-colors"
              style={{
            ...
            >
              ...
              <div className="pt-1">
                ...
                <p className="leading-relaxed text-base sm:text-lg">{k.content}</p>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      >
        <div className="glass-card rounded-3xl p-5 sm:p-7 mb-5 border-secondary/40">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent/20 text-accent font-bold text-sm mb-3">
            ...
            親子討論：{episodeData.FamilyDiscussion.topic}
          </div>
          <p className="leading-relaxed text-base sm:text-lg">
            {episodeData.FamilyDiscussion.description}
          </p>
        </div>

        <Collapsible label="親子洞見參考">
          <div className="markdown-body text-foreground/90 text-sm sm:text-base">
            <ReactMarkdown>{episodeData.FamilyDiscussion.reference_answer}</ReactMarkdown>
          </div>
        </Collapsible>
